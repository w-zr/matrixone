...
// newANode creates an InsertNode with data in memory.
func newANode(
	tbl *txnTable,
...
func (n *anode) FillBlockView(
	batch *containers.Batch, colIdxes []int, mp *mpool.MPool,
) (err error) {
	for _, colIdx := range colIdxes {
		orig := n.data.Vecs[colIdx]
		batch.AddAnonymousVector(orig.CloneWindow(0, orig.Length(), mp))
	}
	batch.Deletes = n.data.Deletes
	return
}
func (n *anode) FillColumnView(batch *containers.Batch, colIdx int, mp *mpool.MPool) (err error) {
	orig := n.data.Vecs[colIdx]
	batch.AddAnonymousVector(orig.CloneWindow(0, orig.Length(), mp))
	batch.Deletes = n.data.Deletes
	return
...
func (n *anode) GetColumnDataByIds(
	...
	view = containers.NewBatchWithCapacity(len(colIdxes))
	err = n.FillBlockView(view, colIdxes, mp)
	return
}

func (n *anode) GetColumnDataById(ctx context.Context, colIdx int, mp *mpool.MPool) (batch *containers.Batch, err error) {
	batch = containers.NewBatchWithCapacity(1)
	err = n.FillColumnView(batch, colIdx, mp)
	return
}
