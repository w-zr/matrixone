...
func TestHandle_HandlePreCommitWriteS3(t *testing.T) {
	...
	for _, view := range physicals {
		...
		bat.Vecs[0], _ = view.Vecs[2].GetDownstreamVector().Window(0, 5)
	...
	delLoc1 := blockio.EncodeLocation(
		writer.GetName(),
		blocks[0].GetExtent(),
		uint32(physicals[0].Vecs[2].Length()),
		blocks[0].GetID(),
	).String()
	assert.Nil(t, err)
	delLoc2 := blockio.EncodeLocation(
		writer.GetName(),
		blocks[1].GetExtent(),
		uint32(physicals[1].Vecs[2].Length()),
		blocks[1].GetID(),
	).String()
	assert.Nil(t, err)
	delLoc3 := blockio.EncodeLocation(
		writer.GetName(),
		blocks[2].GetExtent(),
		uint32(physicals[2].Vecs[2].Length()),
		blocks[2].GetID(),
	).String()
	assert.Nil(t, err)
	delLoc4 := blockio.EncodeLocation(
		writer.GetName(),
		blocks[3].GetExtent(),
		uint32(physicals[3].Vecs[2].Length()),
		blocks[3].GetID(),
	).String()
	assert.Nil(t, err)
